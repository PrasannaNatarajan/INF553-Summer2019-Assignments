...
def append_set(acc,n):
    acc.add(n)
    return acc

def extend_set(acc,n):
    acc.update(n)
    return acc
...
def apriori_algo(transactions,support,num_rdds):
    ...
    supp_par = support / (num_rdds/len(trans))
    ...
    while len(candidates)!=0:
        ...
        i+=1
        freq_items = filter_items(trans,candidates,supp_par)
        for x in freq_items:
            ret.append((x,i-1))
...
if __name__ == "__main__":
    ...
    users = sc.textFile(input_file_path).filter(lambda x: x!="user_id,business_id").map(lambda x: read_csv(x)).combineByKey(lambda x: [x],lambda acc,n: append_list(acc,n),lambda acc,n: extend_list(acc,n)).filter(lambda x: len(x[1])>filter_threshold).map(lambda x: (x[0],set(x[1]))).persist()#.combineByKey(lambda x: {x},lambda acc,n: append_set(acc,n),lambda acc,n: extend_set(acc,n))

    num_rdds = users.count()

    can = users.mapPartitions(lambda x:apriori_algo(x,support,num_rdds)).collect()
